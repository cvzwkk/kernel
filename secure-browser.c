                   ...
                   const gchar *message)
{
    ...
    switch (type) {
        case SEC_EVENT_INFO:            prefix = "[INFO]";                 break;
        case SEC_EVENT_VIOLATION:       prefix = "[SECURITY VIOLATION]";   break;
        case SEC_EVENT_THEFT_ATTEMPT:   prefix = "[THEFT ATTEMPT]";        break;
        case SEC_EVENT_INTRUSION:       prefix = "[INTRUSION DETECTED]";   break;
        case SEC_EVENT_FILE_MOD:        prefix = "[FILE/MODULE MOD]";      break;
        case SEC_EVENT_NETWORK_ANOMALY: prefix = "[NETWORK ANOMALY]";      break;
    }
    ...
    if (console_text_view && GTK_IS_TEXT_VIEW(console_text_view)) {
        GtkTextBuffer *buffer =
            gtk_text_view_get_buffer(GTK_TEXT_VIEW(console_text_view));
...
{
    GChecksum *cs1 = g_checksum_new(G_CHECKSUM_SHA512);

    if (domain_tag != NULL) {
        g_checksum_update(cs1,
                          (const guchar *)domain_tag,
                          strlen(domain_tag));
    }

    if (input != NULL && input_len > 0) {
        g_checksum_update(cs1, input, input_len);
    }

    guint8 digest1[64];
    gsize digest1_len = sizeof(digest1);
    g_checksum_get_digest(cs1, digest1, &digest1_len);
    g_checksum_free(cs1);

    GChecksum *cs2 = g_checksum_new(G_CHECKSUM_SHA512);

    const guint64 domain_salt = G_GUINT64_CONSTANT(0xDEADBEEFCAFE512ULL);

    g_checksum_update(cs2, (const guchar *)&domain_salt, sizeof(domain_salt));
    g_checksum_update(cs2, digest1, sizeof(digest1));
    g_checksum_update(cs2, (const guchar *)&input_len, sizeof(input_len));

    if (domain_tag != NULL) {
        g_checksum_update(cs2,
                          (const guchar *)domain_tag,
                          strlen(domain_tag));
    }
    ...
    g_checksum_get_digest(cs2, digest2, &digest2_len);
    g_checksum_free(cs2);

    for (gsize i = 0; i < output_len; ++i) {
        output[i] = digest1[i % 64] ^
                    digest2[i % 64] ^
                    (guint8)((i * 31u) & 0xffu);
    }
}
    ...
    guint8        output_key[HYBRID_AES_KEY_SIZE])
{
    ...
    if (x25519_secret == NULL || mlkem_secret == NULL ||
        ...
        log_security_event(SEC_EVENT_VIOLATION, "Hybrid-PQ-KDF",
...
compute_file_checksum(const gchar *filepath)
{
    ...
    guint8 digest[64];
    secure_domain_hash((const guint8 *)contents, length,
                       digest, sizeof(digest),
                       "BROWSER-FILE-INTEGRITY-V3");

    gchar *hex = g_malloc0(sizeof(digest) * 2 + 1);
...
{
    gchar *hash = compute_file_checksum(binary_path);

    if (hash != NULL) {
        gchar *message = g_strdup_printf(
            "SHA-512 domain-separated module checksum: %s", hash);
        log_security_event(SEC_EVENT_INFO, "Module-Integrity", message);
        g_free(message);
        g_free(hash);
    } else {
        log_security_event(SEC_EVENT_FILE_MOD, "Module-Integrity",
...
pq_ratchet_cookie_key(PostQuantumSecuredCookie *cookie)
{
    ...
    guint8 combined[72];
    memcpy(combined, cookie->current_pq_state, 64);
    memcpy(combined + 64, &cookie->version_counter,
           sizeof(cookie->version_counter));

    secure_domain_hash(combined, sizeof(combined),
                       cookie->current_pq_state, 64,
                       "COOKIE-RATCHET-V3");
}
...
{
    if (domain == NULL || name == NULL || plain_value == NULL || *domain == '\0')
        return NULL;

    PostQuantumSecuredCookie *cookie = g_new0(PostQuantumSecuredCookie, 1);
    cookie->domain      = g_strdup(domain);
    cookie->cookie_name = g_strdup(name);
    cookie->version_counter = 1;

    if (initial_seed != NULL) {
        memcpy(cookie->current_pq_state, initial_seed,
               sizeof(cookie->current_pq_state));
    } else {
        GRand *rand = g_rand_new();
        for (gsize i = 0; i < 64; ++i)
            cookie->current_pq_state[i] = (guint8)(g_rand_int(rand) & 0xff);
        g_rand_free(rand);
    }

    gsize value_len = strlen(plain_value);
    if (value_len > sizeof(cookie->ciphertext)) {
        log_security_event(SEC_EVENT_VIOLATION, "Cookie-System",
    ...
    for (gsize i = 0; i < value_len; ++i) {
        guint8 domain_mask =
            (guint8)cookie->domain[i % strlen(cookie->domain)];
        cookie->ciphertext[i] =
            ((guint8)plain_value[i] ^ cookie->current_pq_state[i % 64]) ^
            domain_mask;
    }

    cookie->ciphertext_len = value_len;
...
{
    if (cookie == NULL || accessor_domain == NULL)
        return NULL;

    if (g_strcmp0(cookie->domain, accessor_domain) != 0) {
        gchar *alert = g_strdup_printf(
            "Unauthorized domain '%s' attempted to access cookie owned by '%s'.",
            accessor_domain, cookie->domain);
        log_security_event(SEC_EVENT_THEFT_ATTEMPT, "Cookie-Theft-Monitor", alert);
        g_free(alert);
        return g_strdup("[BLOCKED: Cross-Site Cookie Theft Detected]");
    }

    gchar *plain = g_malloc0(cookie->ciphertext_len + 1);
    for (gsize i = 0; i < cookie->ciphertext_len; ++i) {
        guint8 domain_mask =
            (guint8)cookie->domain[i % strlen(cookie->domain)];
        plain[i] = (cookie->ciphertext[i] ^ domain_mask) ^
...
pq_free_cookie(PostQuantumSecuredCookie *cookie)
{
    ...
    g_free(cookie->cookie_name);
    g_free(cookie->domain);
    memset(cookie->ciphertext, 0, sizeof(cookie->ciphertext));
    memset(cookie->current_pq_state, 0, sizeof(cookie->current_pq_state));
    g_free(cookie);
}
...
{
    PostQuantumProfileKey *key = g_new0(PostQuantumProfileKey, 1);

    GRand *rand = g_rand_new();
    guint8 seed[64];
    for (gsize i = 0; i < sizeof(seed); ++i)
        seed[i] = (guint8)(g_rand_int(rand) & 0xff);
    g_rand_free(rand);

    secure_domain_hash(seed, sizeof(seed),
                       key->key_bytes, sizeof(key->key_bytes),
...
free_tab_key(PostQuantumProfileKey *key)
{
    if (key == NULL)
        return;
    memset(key->key_bytes, 0, sizeof(key->key_bytes));
...
recursively_delete_directory(const gchar *dirname)
{
    ...
    while ((filename = g_dir_read_name(dir)) != NULL) {
        gchar *filepath = g_build_filename(dirname, filename, NULL);
...
static void
on_tab_destroy_cleanup(GtkWidget *widget, gpointer user_data)
{
    (void)widget;
    TabContextData *tab = (TabContextData *)user_data;
    if (tab == NULL)
        return;

    log_security_event(SEC_EVENT_INFO, "Auto-Destruct",
                       "Tab closed. Removing temporary profile sandbox and caches.");

    if (tab->data_dir)
        recursively_delete_directory(tab->data_dir);
    if (tab->cache_dir)
        recursively_delete_directory(tab->cache_dir);
...
uri_contains_suspicious_pattern(const gchar *uri)
{
    ...
    static const gchar *patterns[] = {
        "<script>", "union select", "../../../",
        "%3cscript", "%3Cscript", "../", NULL
    };

    for (gsize i = 0; patterns[i] != NULL; ++i) {
        if (g_strstr_len(uri, -1, patterns[i]))
            return TRUE;
    }
    return FALSE;
}
...
static gboolean
zero_trust_policy_decision_cb(WebKitWebView *web_view,
                              ...
                              gpointer user_data)
{
    ...
    if (g_str_has_prefix(uri, "http://") &&
        ...
        gchar *message = g_strdup_printf(
            "Cleartext HTTP navigation blocked: %s", uri);
        log_security_event(SEC_EVENT_NETWORK_ANOMALY,
        ...
        const gchar *block_html =
            ...
            "padding:40px;border:1px solid #30363d;border-radius:8px;'>"
            "<h2 style='color:#f85149;'>Network Traffic Blocked</h2>"
            "<p>Cleartext HTTP navigation is disabled by the browser "
            "security policy.</p></div></body></html>";

        webkit_web_view_load_alternate_html(web_view, block_html, uri, NULL);
    ...
    if (uri_contains_suspicious_pattern(uri)) {
        gchar *message = g_strdup_printf(
            "Suspicious URI attack signature detected: %s", uri);
        log_security_event(SEC_EVENT_INTRUSION,
                           "Intrusion-Detection", message);
        g_free(message);
    }

    /* Internal search router */
    if (g_str_has_prefix(uri, "internal://search")) {
        const gchar *query = strchr(uri, '?');
        if (query != NULL)
            query++;
        else
            query = "";

        const gchar *q = g_strstr_len(query, -1, "q=");
        if (q != NULL) {
            q += 2;
            gchar *decoded = g_uri_unescape_string(q, NULL);
            if (decoded == NULL)
                decoded = g_strdup(q);

            gchar *escaped = g_uri_escape_string(decoded, NULL, TRUE);
            gchar *search_url = g_strdup_printf(
                "https://html.duckduckgo.com/html/?q=%s", escaped);

            log_security_event(SEC_EVENT_INFO, "Secure-Search",
...
static void
add_autodestroy_tab(GtkNotebook *nb, const gchar *initial_uri)
{
    gchar *unique_id = g_uuid_string_random();
    gchar *data_dir  = g_build_filename(g_get_user_cache_dir(),
    ...
    TabContextData *tab = g_new0(TabContextData, 1);
    tab->data_dir  = g_strdup(data_dir);
    tab->cache_dir = g_strdup(cache_dir);
    tab->pq_key    = pq_generate_tab_key();
    ...
    webkit_website_data_manager_set_ephemeral(manager, TRUE);
    ...
    webkit_settings_set_tls_errors_are_fatal(settings, TRUE);

    /* Correct WebKitGTK API (no new_with_context_and_settings) */
    WebKitWebView *web_view =
    ...
    g_signal_connect(web_view, "decide-policy",
                     G_CALLBACK(zero_trust_policy_decision_cb), tab);

    GtkWidget *scrolled = gtk_scrolled_window_new(NULL, NULL);
    tab->scrolled_window = scrolled;
    gtk_container_add(GTK_CONTAINER(scrolled), GTK_WIDGET(web_view));

    GtkWidget *tab_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 4);
    GtkWidget *tab_label = gtk_label_new("PQ Secure Tab");
    gtk_box_pack_start(GTK_BOX(tab_box), tab_label, TRUE, TRUE, 0);
    gtk_widget_show_all(tab_box);

    gint page_num = gtk_notebook_append_page(nb, scrolled, tab_box);
    gtk_notebook_set_current_page(nb, page_num);

    g_signal_connect(scrolled, "destroy",
                     G_CALLBACK(on_tab_destroy_cleanup), tab);

    const gchar *start_page =
        "<!doctype html><html><head><meta charset='utf-8'>"
        "<title>Secure Browser</title></head>"
        "<body style='background:#0d1117;color:#c9d1d9;"
        "font-family:sans-serif;display:flex;justify-content:center;"
        "align-items:center;height:100vh;margin:0;'>"
        "<div style='background:#161b22;border:1px solid #30363d;"
        "padding:40px;border-radius:8px;text-align:center;max-width:650px;'>"
        "<h2 style='color:#58a6ff;'>Secure Browser</h2>"
        "<p style='color:#8b949e;'>Private tab isolation and network "
        "security monitoring are active.</p>"
        "<form action='internal://search' method='GET'>"
        "<input type='text' name='q' "
        "placeholder='Enter secure search query...' "
        "style='padding:10px;width:90%;background:#0d1117;color:#fff;"
        "border:1px solid #30363d;border-radius:4px;margin-top:10px;'>"
        "</form></div></body></html>";

    if (initial_uri != NULL && *initial_uri != '\0')
        webkit_web_view_load_uri(web_view, initial_uri);
    else
        webkit_web_view_load_alternate_html(web_view, start_page,
                                            "internal://start", NULL);
...
main(int argc, char *argv[])
{
    ...
    if (argc > 0 && argv[0] != NULL)
        verify_module_integrity(argv[0]);
    ...
    gtk_paned_pack1(GTK_PANED(vpaned), browser_box, TRUE, FALSE);

    notebook = gtk_notebook_new();
    gtk_box_pack_start(GTK_BOX(browser_box), notebook, TRUE, TRUE, 0);

    /* Security console */
    GtkWidget *console_frame =
        gtk_frame_new("Live Security Event & Intrusion Detection Console");
    GtkWidget *console_scrolled = gtk_scrolled_window_new(NULL, NULL);
    gtk_widget_set_size_request(console_frame, -1, 200);
    ...
    /* Modern replacement for the deprecated gtk_widget_override_font */
    ...
    gtk_container_add(GTK_CONTAINER(console_scrolled), console_text_view);
    gtk_container_add(GTK_CONTAINER(console_frame), console_scrolled);
    gtk_paned_pack2(GTK_PANED(vpaned), console_frame, FALSE, TRUE);

    add_autodestroy_tab(GTK_NOTEBOOK(notebook), NULL);

    log_security_event(SEC_EVENT_INFO, "System-Core",
                       "Secure browser initialized.");
    log_security_event(SEC_EVENT_INFO, "Integrity",
                       "Domain-separated SHA-512 integrity subsystem initialized.");
    log_security_event(SEC_EVENT_INFO, "Network-Security",
                       "Cleartext HTTP navigation blocking enabled.");
    log_security_event(SEC_EVENT_INFO, "Privacy",
                       "Ephemeral WebKit tab profile initialized.");
    log_security_event(SEC_EVENT_INFO, "Intrusion-Detection",
...
# gcc -O2 -Wall -Wextra -o secure_browser secure_browser.c $(pkg-config --cflags --libs gtk+-3.0 webkit2gtk-4.1 libsoup-3.0) -lcrypto
